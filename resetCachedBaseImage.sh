...
_new_image=${_registry_name}microsoft/microsoft/dotnet:2.1-aspnetcore-runtime

docker build \
  -f runtime/lock.Dockerfile \
  -t $_new_image \
...
_new_image=${_registry_name}microsoft/dotnet:${_base_image_version}-sdk

docker build \
  -f sdk/lock.Dockerfile \
  -t $_new_image \
  --build-arg BASE_IMAGE_VERSION=${_base_image_version} \
  --build-arg IMAGE_BUILD_DATE=`date +%Y%m%d-%H%M%S` \
  ./sdk
